...
Linux.FrameColor = Color3.fromRGB(27, 27, 27)
...
function Linux:Notify(config)
    ...
    local notificationWidth = math.max(baseWidth, maxTextWidth + 40)
...
function Linux.Create(config)
    ...
    if config.OthersConfig then
        if config.OthersConfig.TabHighlight ~= nil then
            Linux.TabHighlight = config.OthersConfig.TabHighlight
        end
        if config.OthersConfig.DropdownBar ~= nil then
            Linux.DropdownBar = config.OthersConfig.DropdownBar
        end
    end
    ...
    if config.ToggleEnabled ~= false then
        ...
        local connection2 = globalConnectionManager:addConnection(ToggleButton.InputChanged:Connect(function(input)
    ...
    local connection6 = globalConnectionManager:addConnection(ResizeHandle.InputChanged:Connect(function(input)
    ...
    local connection9 = globalConnectionManager:addConnection(TopBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            isDraggingMain = true
    ...
    local connection10 = globalConnectionManager:addConnection(TopBar.InputChanged:Connect(function(input)
    ...
    local connection11 = globalConnectionManager:addConnection(TopBar.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
    ...
    function LinuxLib.Tab(config)
        ...
        local function SelectTab()
            ...
            TabBtn.BackgroundTransparency = 0.80
            ...
            local function UpdateToggle(thisToggle)
                ...
                if thisTrack and thisKnob then
                    ...
                    if currentState then
                        if Linux.CurrentAccent == "Black & White" then
                            local tween1 = TweenService:Create(thisTrack, tween, {BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0})
                            ...
                            local tween1 = TweenService:Create(thisTrack, tween, {BackgroundColor3 = Linux.Accents[Linux.CurrentAccent], BorderSizePixel = 0})
                        ...
                        local tween1 = TweenService:Create(thisTrack, tween, {BackgroundColor3 = Color3.fromRGB(17, 17, 18), BorderSizePixel = 1})
    ...
    if config.ConfigSave ~= false then
        ...
        SettingsTab.Section({Name = "Accent"})
